<script lang="ts">
	...
	// Réagir aux changements dans les données du post
	$: {
	  if (post) {
		if (post.tags?.length) {
		  metaKeywords = post.tags.concat(keywords);
		}
  ...
  <div class="article-layout">
	...
	<main style="margin-top:100px;">
	  <article id="article-content" in:fly={{ y: 100, duration: 1000 }}>
		<div class="header">
		  {#if post}
			{#if post.postlang === urlLang}
			  <h1>{post.title}</h1>
			  <div class="note">{$_('/.blog.publierle')} {dateformat(post.date, 'UTC:dd/mm/yyyy')}</div>
			  {#if post.updated}
				<div class="note">{$_('/.blog.modifierle')} {dateformat(post.updated, 'UTC:dd/mm/yyyy')}</div>
			  {/if}
			  {#if post.readingTime}
				<div class="note">{post.readingTime.replace(/\bread\b/gi, '').trim()}</div>
			  {/if}
			  {#if post.tags?.length}
				<div class="tags">
